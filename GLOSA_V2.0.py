...
def accel_rate(speed, driver):
	...
	return 2.5 * exp(-0.12 * speed) * driver
	...
	while t < test_dur or orphan_in_sim > 0 or v2x_in_sim > 0 or (t-0.1) % x_grid != 0:
		...
		for veh in list(orphan_vehs):
			...
			if veh.state != 'unspawned' and veh.state != 'despawned':
				...
				gap = stoping_gap(veh.speed, veh.driver)
				...
				if veh.state == 'cruise':
					if veh_in_front.state == 'despawned':
							veh.speed += cruise_mod * accel * veh.driver * t_step
				...
				if veh.state == 'accel':
					...
					if veh.speed > 0.97 * veh.driver * speed_limit_mps or veh.speed > 0.97 * veh_in_front.driver * speed_limit_mps:
						veh.state = 'cruise'
			...
			veh_in_front.state = veh.state
			veh_in_front.dist = veh.dist
			veh_in_front.driver = veh.driver
			veh_in_front.speed = veh.speed
		...
		v2x_pos = 0
		v2x_in_front.state = 'despawned'
		v2x_in_front.dist = departure
		v2x_in_front.driver = 1.0
		v2x_in_front.speed = speed_limit_mps
		...
		for veh in list(v2x_vehs):
			...
			if veh.state != 'unspawned' and veh.state != 'despawned':
				...
				hys = hysteresis(veh.speed, veh.driver)
				b_dist = braking_dist(veh.speed, veh.driver)
				vif_b_dist = braking_dist(v2x_in_front.speed, v2x_in_front.driver)
				...
				if veh.state == 'cruise':
					if veh.dist > -spat_range and veh.dist < 0:
						veh.state = 'glosa'
					elif v2x_in_front.state == 'despawned':
							veh.speed += cruise_mod * accel * veh.driver * t_step
							if veh.speed > speed_limit_mps * veh.driver:
								veh.speed = speed_limit_mps * veh.driver
					# check and control safe cruising headway if inside soft headway limit
					elif v2x_in_front.state == 'cruise':
						# decel if inside safe braking limit
						if veh.dist + b_dist + gap - vif_b_dist > v2x_in_front.dist:
							veh.speed -= brake * veh.driver * t_step
						elif veh.dist + b_dist + gap - vif_b_dist + hys > v2x_in_front.dist:
							veh.speed -= (a + b * veh.speed + c * veh.speed**2) / m * t_step
						# accelerate to cruising speed if outside safe braking limit
						elif veh.dist + b_dist + gap - vif_b_dist < v2x_in_front.dist and veh.speed < speed_limit_mps * veh.driver:
							veh.speed += cruise_mod * accel * veh.driver * t_step
							if veh.speed > speed_limit_mps * veh.driver:
								veh.speed = speed_limit_mps *  veh.driver
	
					# check safe braking distances
					if (v2x_in_front.state == 'brake' or v2x_in_front.state == 'stop' or v2x_in_front.state == 'accel') and veh.dist + b_dist + gap - vif_b_dist > v2x_in_front.dist:
						veh.state = 'brake'
					if t_light_state == 'red' and veh.dist + b_dist + gap > 0 and veh.dist < 0:
						veh.state = 'brake'
					if t_light_state == 'amb' and veh.dist + b_dist + gap > 0 and veh.dist < 0 and -veh.dist / veh.speed > t_light_data[int(t*10)][2]:
				...
				if veh.state == 'glosa':
					...
					#print v2x_pos, glosa_min, glosa_max
					# check and control safe cruising headway if inside soft headway limit
					elif v2x_in_front.state == 'glosa' or v2x_in_front.state == 'cruise' or v2x_in_front.state == 'despawned' or v2x_in_front.state == 'brake':
						# decel if inside safe braking limit
						if veh.dist + b_dist + gap - vif_b_dist > v2x_in_front.dist or veh.speed > glosa_max:
							veh.speed -= brake * veh.driver * t_step
						elif veh.dist + b_dist + gap - vif_b_dist + hys > v2x_in_front.dist:
							veh.speed -= (a + b * veh.speed + c * veh.speed**2) / m * t_step
#						elif veh.dist + b_dist + gap - vif_b_dist > v2x_in_front.dist:
#							veh.speed -= brake * veh.driver * t_step
#						elif veh.dist + b_dist + gap - vif_b_dist + hys > v2x_in_front.dist:
#							veh.speed -= (a + b * veh.speed + c * veh.speed**2) / m * t_step
				...
				if veh.state == 'brake':
					...
					if (v2x_in_front.state == 'brake' or v2x_in_front.state == 'stop' or v2x_in_front.state == 'accel') and veh.dist + b_dist - vif_b_dist > v2x_in_front.dist:
						veh.speed -= brake * veh.driver * t_step
					
					#if (v2x_in_front.state == 'brake' or v2x_in_front.state == 'stop' or v2x_in_front.state == 'accel') and veh.dist + b_dist + hys - vif_b_dist < v2x_in_front.dist:
						#veh.state = 'accel'
					...
					if t_light_state =='grn' or t_light_state == 'red_amb':
						if v2x_in_front.state == 'despawned':
							veh.state = 'accel'
						elif (v2x_in_front.state == 'cruise') and (veh.dist + gap + b_dist - vif_b_dist + hys < v2x_in_front.dist):
							veh.state = 'accel'
						elif (v2x_in_front.state == 'accel') and (veh.dist + gap + b_dist - vif_b_dist + hys < v2x_in_front.dist):
							veh.state = 'accel'
				...
				if (veh.state == 'stop' and (t_light_state == 'grn' or t_light_state == 'red_amb')) and (v2x_in_front.state == 'accel' or v2x_in_front.state == 'cruise' or v2x_in_front.state == 'despawned'):
					veh.state = 'accel'
				...
				if veh.state == 'accel':
					...
					#if (v2x_in_front.state == 'cruise' or v2x_in_front.state == 'glosa') and veh.dist + gap + b_dist - vif_b_dist + hys < v2x_in_front.dist:
						#veh.speed += accel * veh.driver * t_step
						
					if v2x_in_front.state == 'accel' or v2x_in_front.state == 'cruise' or v2x_in_front.state == 'glosa':
						if veh.dist + gap + b_dist - vif_b_dist + hys < v2x_in_front.dist:
							veh.speed += accel * veh.driver * t_step
						elif veh.dist + b_dist + gap - vif_b_dist + hys > v2x_in_front.dist:
							veh.speed -= (a + b * veh.speed + c * veh.speed**2) / m * t_step
						elif veh.dist + b_dist + gap - vif_b_dist > v2x_in_front.dist:
					...
					if veh.speed > 0.97 * veh.driver * speed_limit_mps or veh.speed > 0.97 * v2x_in_front.driver * speed_limit_mps:
							veh.state = 'cruise'
						
					if (v2x_in_front.state == 'brake' or v2x_in_front.state == 'stop') and veh.dist + b_dist + gap - vif_b_dist > v2x_in_front.dist:
						veh.state = 'brake'
						veh.speed -= brake * veh.driver * t_step
					if t_light_state == 'red' and veh.dist + b_dist + gap > 0 and veh.dist < 0:
						veh.state = 'brake'
				...
				if veh.dist > departure:
					...
					veh.state ='despawned'
					v2x_in_sim -= 1
					#print 'despawn v2x', t, v2x_pos, v2x_in_sim
					
			v2x_in_front.state = veh.state
			v2x_in_front.dist = veh.dist
			v2x_in_front.driver = veh.driver
			v2x_in_front.speed = veh.speed
			
			v2x_veh_data[v2x_pos].append([t, round(veh.speed,3), round(veh.dist,3), veh.state, 0, 0, 0])
	...
	#Smooth velocity as moving average of 5 points
		#Calculate first 3 points 
		orphan_veh_data[v][0][4] = (orphan_veh_data[v][0][1] + orphan_veh_data[v][1][1] + orphan_veh_data[v][2][1]) / 3
		orphan_veh_data[v][1][4] = (orphan_veh_data[v][0][1] + orphan_veh_data[v][1][1] + orphan_veh_data[v][2][1] + orphan_veh_data[v][3][1]) / 4
		v2x_veh_data[v][0][4] = (v2x_veh_data[v][0][1] + v2x_veh_data[v][1][1] + v2x_veh_data[v][2][1]) / 3
		v2x_veh_data[v][1][4] = (v2x_veh_data[v][0][1] + v2x_veh_data[v][1][1] + v2x_veh_data[v][2][1] + v2x_veh_data[v][3][1]) / 4
		
		#Loop for points 4 thru 4th from end
		for i in range(2, int(sim_time*10)-1):
			orphan_veh_data[v][i][4] = (orphan_veh_data[v][i-2][1] + orphan_veh_data[v][i-1][1] + orphan_veh_data[v][i][1] + orphan_veh_data[v][i+1][1] + orphan_veh_data[v][i+2][1]) / 5
			v2x_veh_data[v][i][4] = (v2x_veh_data[v][i-2][1] + v2x_veh_data[v][i-1][1] + v2x_veh_data[v][i][1] + v2x_veh_data[v][i+1][1] + v2x_veh_data[v][i+2][1]) / 5

		#Calculate last 3 points 			
		orphan_veh_data[v][int(sim_time*10)][4] = (orphan_veh_data[v][int(sim_time*10)][1] + orphan_veh_data[v][int(sim_time*10)-1][1] + orphan_veh_data[v][int(sim_time*10)-2][1]) / 3
		orphan_veh_data[v][int(sim_time*10)-1][4] = (orphan_veh_data[v][int(sim_time*10)][1] + orphan_veh_data[v][int(sim_time*10)-1][1] + orphan_veh_data[v][int(sim_time*10)-2][1] + orphan_veh_data[v][int(sim_time*10)-3][1]) / 4
		v2x_veh_data[v][int(sim_time*10)][4] = (v2x_veh_data[v][int(sim_time*10)][1] + v2x_veh_data[v][int(sim_time*10)-1][1] + v2x_veh_data[v][int(sim_time*10)-2][1]) / 3
		v2x_veh_data[v][int(sim_time*10)-1][4] = (v2x_veh_data[v][int(sim_time*10)][1] + v2x_veh_data[v][int(sim_time*10)-1][1] + v2x_veh_data[v][int(sim_time*10)-2][1] + v2x_veh_data[v][int(sim_time*10)-3][1]) / 4
		...
		for i in range(1, int(sim_time*10)):
			...
			orphan_veh_data[v][i][5] = round((a + b * orphan_veh_data[v][i][4] + c * orphan_veh_data[v][i][4]**2 + orphan_inertia_force) * (orphan_veh_data[v][i][2] - orphan_veh_data[v][i-1][2]), 3)
			if orphan_veh_data[v][i][5] < 0.0:
				orphan_veh_data[v][i][5] = 0.000
			orphan_energy += orphan_veh_data[v][i][5] * 2.778 * 10**-7
			v2x_veh_data[v][i][5] = round((a + b * v2x_veh_data[v][i][4] + c * v2x_veh_data[v][i][4]**2 + v2x_inertia_force) * (v2x_veh_data[v][i][2] - v2x_veh_data[v][i-1][2]), 3)
			if v2x_veh_data[v][i][5] < 0.0:
				v2x_veh_data[v][i][5] = 0.000
			v2x_energy += v2x_veh_data[v][i][5] * 2.778 * 10**-7
...
#	data_element.append(t_light_data[i][0])		
#	for v in range(0, veh_num):			
#		data_element.append(orphan_veh_data[v][i][1])		# Speed
#		data_element.append(v2x_veh_data[v][v][1])			# Speed
		
#	data_element.append(t_light_data[i][0])		
#	for v in range(0, veh_num):						
#		data_element.append(orphan_veh_data[v][i][3])		# State
#		data_element.append(v2x_veh_data[v][i][3])			# State

	data_element.append(t_light_data[i][0])		
	for v in range(0, veh_num):						
		data_element.append(orphan_veh_data[v][i][4])		# Smoothed Speed
		data_element.append(v2x_veh_data[v][i][4])			# Smoothed Speed

	data_element.append(t_light_data[i][0])		
	for v in range(0, veh_num):						
		data_element.append(orphan_veh_data[v][i][5])		# Energy J
		data_element.append(v2x_veh_data[v][i][5])			# Energy J
